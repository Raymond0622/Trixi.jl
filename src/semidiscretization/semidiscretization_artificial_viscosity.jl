...
"""
    SemidiscretizationArtificialViscosity
...
"""
    SemidiscretizationArtificialViscosity(mesh, both_equations, initial_condition, solver;
...
function remake(semi::SemidiscretizationArtificialViscosity;
...
function Base.show(io::IO, semi::SemidiscretizationArtificialViscosity)
    @nospecialize semi # reduce precompilation time

    print(io, "SemidiscretizationArtificialViscosity(")
    print(io, semi.mesh)
...
function Base.show(io::IO, ::MIME"text/plain",
    ...
    if get(io, :compact, false)
        ...
        summary_header(io, "SemidiscretizationArtificialViscosity")
...
@inline Base.ndims(semi::SemidiscretizationArtificialViscosity) = ndims(semi.mesh)

@inline function nvariables(semi::SemidiscretizationArtificialViscosity)
    nvariables(semi.equations)
end

@inline Base.real(semi::SemidiscretizationArtificialViscosity) = real(semi.solver)

# retain dispatch on hyperbolic equations only
@inline function mesh_equations_solver_cache(semi::SemidiscretizationArtificialViscosity)
    @unpack mesh, equations, solver, cache = semi
    return mesh, equations, solver, cache
end

function calc_error_norms(func, u_ode, t, analyzer,
                          semi::SemidiscretizationArtificialViscosity, cache_analysis)
...
function compute_coefficients(t, semi::SemidiscretizationArtificialViscosity)
    # Call `compute_coefficients` in `src/semidiscretization/semidiscretization.jl`
    compute_coefficients(semi.initial_condition, t, semi)
end

function compute_coefficients!(u_ode, t, semi::SemidiscretizationArtificialViscosity)
...
"""
    semidiscretize(semi::SemidiscretizationArtificialViscosity, tspan;
...
"""
function semidiscretize(semi::SemidiscretizationArtificialViscosity, tspan;
                        jac_prototype_parabolic::Union{AbstractMatrix, Nothing} = nothing,
                        colorvec_parabolic::Union{AbstractVector, Nothing} = nothing,
                        reset_threads = true)
...
function rhs_parabolic!(du_ode, u_ode, semi::SemidiscretizationArtificialViscosity, t)
...
"""
    linear_structure(semi::SemidiscretizationArtificialViscosity;
                     t0 = zero(real(semi)))
...
"""
function linear_structure(semi::SemidiscretizationArtificialViscosity;
                          t0 = zero(real(semi)))
...
function _jacobian_ad_forward(semi::SemidiscretizationArtificialViscosity, t0, u0_ode,
                              du_ode, config)
...
"""
    linear_structure_parabolic(semi::SemidiscretizationArtificialViscosity;
                               t0 = zero(real(semi)))
...
"""
function linear_structure_parabolic(semi::SemidiscretizationArtificialViscosity;
                                    t0 = zero(real(semi)))
...
"""
    jacobian_ad_forward_parabolic(semi::SemidiscretizationArtificialViscosity;
                                  t0 = zero(real(semi)),
                                  u0_ode = compute_coefficients(t0, semi))

Uses the *parabolic part* of the right-hand side operator of the [`SemidiscretizationArtificialViscosity`](@ref) `semi`
...
"""
function jacobian_ad_forward_parabolic(semi::SemidiscretizationArtificialViscosity;
                                       t0 = zero(real(semi)),
                                       u0_ode = compute_coefficients(t0, semi))
...
function _jacobian_ad_forward_parabolic(semi::SemidiscretizationArtificialViscosity, t0,
                                        u0_ode, du_ode, config)
    new_semi = remake(semi, uEltype = eltype(config))
